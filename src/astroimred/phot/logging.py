"""Logging configuration for astroimred.phot.

This module provides a centralized logging infrastructure for astroimred.phot,
...
>>> fip.enable_console_logging(level=logging.INFO)
>>> # Or use DEBUG for detailed diagnostics
>>> fip.enable_console_logging(level=logging.DEBUG)
>>> # Standard logging configuration also works
>>> logging.getLogger("astroimred.phot").setLevel(logging.WARNING)
"""
import logging
...
# When imported as a package, this will be 'astroimred.phot'
logger = logging.getLogger("astroimred.phot")
...
def set_log_level(level):
    """Set the log level for astroimred.phot.
    ...
    >>> import astroimred.phot as fip
    >>> import logging
    >>> fip.set_log_level(logging.DEBUG)
    >>> fip.set_log_level('INFO')  # Also accepts strings
    """
...
def enable_console_logging(level=logging.INFO, format=None):
    ...
    when using astroimred.phot interactively. By default, the library is silent
