...
IMUTIL_USE_NUMBA is imported from fir.imutil (can be set via fir.IMUTIL_USE_NUMBA).
"""
...
def reject_sigclip_numba(
    ...
    mask = np.ascontiguousarray(mask) if mask is not None else np.zeros(arr.shape, dtype=np.bool_)
    use_median = "median" in str(cenfunc).lower() if cenfunc is not None else True
    return reject_sigclip_3d(
        arr, mask, sigma_lower, sigma_upper, maxiters, ddof, nkeep, maxrej, use_median, irafmode,
        ccdclip, rdnoise_ref, snoise_ref, scale_ref, zero_ref
    )
