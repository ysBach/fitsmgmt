...
try:
    _ureduce = np.lib.function_base._ureduce
except AttributeError:
    from numpy.lib._function_base_impl import _ureduce


def _ureduce_compat(a, func, **kwargs):
    r = _ureduce(a, func=func, **kwargs)
...
try:
    _median_nancheck = np.lib.utils._median_nancheck
except AttributeError:
    from numpy.lib._utils_impl import _median_nancheck


def _median_nancheck_compat(data, result, axis, out=None):
    try:
        return _median_nancheck(data, result, axis, out)
    except TypeError:
        return _median_nancheck(data, result, axis)
...
def _median(a, axis=None, out=None, overwrite_input=False, choose="mean"):
    ...
    if np.issubdtype(a.dtype, np.inexact) and sz > 0:
        ...
        return _median_nancheck_compat(part, rout, axis, out)
    else:
...
def _ma_median(a, axis=None, out=None, overwrite_input=False, choose="mean"):
    ...
    if asorted.ndim == 1:
        ...
        if np.issubdtype(asorted.dtype, np.inexact) and asorted.size > 0:
            ...
            s = _median_nancheck_compat(asorted, s, axis, out)
        else:
            s = mid.mean(out=out)
